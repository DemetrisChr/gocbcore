...
func (suite *StandardTestSuite) TestZombieLoggerComponentCancelledOps() {
	...
	for i := 0; i < 20; i++ {
		...
		op, err := agent.Get(GetOptions{
			Key:            []byte("zombie-test"),
			CollectionName: suite.CollectionName,
			ScopeName:      suite.ScopeName,
		}, func(res *GetResult, err error) {
			suite.Assert().Nil(res)
			suite.Assert().ErrorIs(err, ErrRequestCanceled)
			close(doneCh)
		})
		suite.Require().NoError(err)
	...
	suite.Require().Eventually(
		func() bool {
			agent.zombieLogger.zombieLock.Lock()
			zombieCount = len(agent.zombieLogger.zombieOps)
			agent.zombieLogger.zombieLock.Unlock()
